...
b_futs <- b_cupom %>%
  ...
  mutate(bcb = 0)
```
Function to tody and relevel sectors as factors for ploting:
...
plot_inst2 <- b_futs_tidy %>%
  ...
  scale_fill_viridis_d('',
                       labels = groups_labels) +
  theme_bw(base_family = "MS Reference Sans Serif") +
  labs(x='', y='US$ Billion', subtitle = 'B: DDI and Future Markets') +
  theme(legend.position = 'bottom',
        plot.caption = element_text(hjust = 0),
        axis.title.x = element_blank()) +
  guides(fill=guide_legend(nrow=2,byrow=TRUE))
...
ggpubr::ggarrange(plot_inst1, plot_inst2, 
                  nrow = 2, ncol = 1,
                  common.legend = T,
                  legend = 'bottom') 
```
...
```{r}
b3_volume %>%
  ggplot(aes(x = date, y = vol_usd)) +
  geom_col(aes(fill = mercado)) +
  geom_col(aes(fill = mercado, color = mercado), show.legend = FALSE) +
  theme_bw() +
...
```{r}
ggplot(data = d_swaps1, aes(x = date)) +
...
fig5data %>%
  ...
  scale_fill_manual("", values = c('dodgerblue3'), labels = c('High-coupon FX Swaps')) +
...
ggpubr::ggarrange(panel_ab, windows_rsquared_plot, nrow = 2, heights = c(1.3, .8), common.legend = F)
